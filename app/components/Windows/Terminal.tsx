...
export default function Terminal() {
  ...
  const getOutputForCommand = (cmd: string): string | undefined => {
    ...
    if (normalizedCmd === 'who') return `Md Foysal  console  ${new Date().toLocaleString('en-US', { month: 'short', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false })}`;
    ...
    <div ref={terminalRef} className="flex-1 font-mono text-sm rounded-lg p-2 overflow-y-auto bg-background/20">
      {lines.map((line, idx) => (
        <div key={idx} className="mb-1">
          ...
          )}>macbook@mdfoysalcysec-Machine ~ %</span> {line.command}
          {line.output && (
            <div className={clsx(
              "whitespace-pre-wrap",
      ...
      <div className="flex items-center">
        <span className={clsx(
          "select-none",
          theme === "dark" ? "text-green-400" : "text-green-700"
        )}>macbook@mdfoysalcysec-Machine ~ %</span>
        <input
          autoFocus
          type="text"
          value={current}
